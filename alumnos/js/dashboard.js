...
function mostrarDetallesCurso(curso) {
  ...
  estadoTexto = 'Pagada';
      ...
      // Verificar si el tipopago es siro o coin para mostrar botón de pagar
      const tipoPago = (curso.tipopago || '').toLowerCase();
      const mostrarBotonPagar = tipoPago === 'siro' || tipoPago === 'coin';

      if (mostrarBotonPagar && !pagada) {
        // Mostrar botón de pagar cuota para siro y caoin
        if (payButton) {
          payButton.classList.remove('hidden');
          payButton.textContent = 'Pagar cuota';
          payButton.onclick = () => {
            // Función para pagar cuota
            pagarCuota(cuotaMostrar, curso);
          };
        }
        
        // Hacer lo mismo para el botón móvil
        const payButtonMobile = document.getElementById('payButtonMobile');
        if (payButtonMobile) {
          payButtonMobile.classList.remove('hidden');
          payButtonMobile.textContent = 'Pagar cuota';
          payButtonMobile.onclick = () => {
            pagarCuota(cuotaMostrar, curso);
          };
        }
      } else {
        // Ocultar botón de pago (comportamiento normal)
        if (payButton) payButton.classList.add('hidden');
        const payButtonMobile = document.getElementById('payButtonMobile');
        if (payButtonMobile) payButtonMobile.classList.add('hidden');
      }
    ...
    const entidadExamen = localStorage.getItem('lastLoginCountry');
    const esAutogestionable = curso.autogestion == 1;
    const mostrarDerechoExamen = !['py', 'uy'].includes(entidadExamen) && !esAutogestionable;

    // Ocultar card de progreso académico para cursos autogestionables
    const progressCard = document.getElementById('progressCard');
    if (progressCard) {
      if (esAutogestionable) {
    ...
    // Para PY, UY o cursos autogestionables: ocultar completamente las tarjetas de derecho de examen
    if (!mostrarDerechoExamen) {
      if (examFeeCard) {
        examFeeCard.classList.add('hidden');
        examFeeCard.classList.remove('lg:block');
      }
      const examFeeMobileCardPyUy = document.getElementById('examFeeMobileCard');
      if (examFeeMobileCardPyUy) examFeeMobileCardPyUy.classList.add('hidden');
    }

    const cuotaExamen = curso.cuotas.find(c => c.cuota === '99');
    if (cuotaExamen && examFeeCard && mostrarDerechoExamen) {
        ...
        if (puedeAbonarExamen) {
          ...
          const handleExamFeePayment = () => {
              ...
              if (cuotaExamen.linkpago99 && cuotaExamen.linkpago99.trim() !== '') {
                ...
                window.open(cuotaExamen.linkpago99, '_self');
                return;
              }
              ...
              if (cuotaExamen.linkpago && cuotaExamen.linkpago.data && cuotaExamen.linkpago.data.link) {
                ...
                window.open(cuotaExamen.linkpago.data.link, '_self');
                return;
              }
  ...
  // Card de datos del Centro
  const centroCard = document.getElementById('centroCard');
  if (centroCard) {
    const entidadCentro = localStorage.getItem('lastLoginCountry');
    const nom  = (curso.centronom  || window.userData?.centronom  || '').trim();
    const dir  = (curso.centrodir  || window.userData?.centrodir  || '').trim();
    const city = (curso.centrocity || window.userData?.centrocity || '').trim();
    const tel  = (curso.centrotel  || window.userData?.centrotel  || '').trim();
    const mail = (curso.centromail || window.userData?.centromail || '').trim();
    ...
    centroCard.innerHTML = `
      <div class="flex items-center gap-4 mb-4">
        <div class="w-12 h-12 bg-indigo-100 dark:bg-indigo-900 rounded-full flex items-center justify-center">
          <i class="fa-solid fa-building text-indigo-600 dark:text-indigo-300 text-2xl"></i>
        </div>
        <h4 class="text-lg font-bold text-gray-900 dark:text-text-dark">Mi Centro</h4>
      </div>
      <div class="flex flex-col gap-3 text-sm">
        ${nom ? `<div class="flex items-start gap-2"><i class="fa-solid fa-store text-gray-400 mt-0.5 w-4"></i><span class="text-gray-900 dark:text-text-dark font-medium">${nom}</span></div>` : ''}
        ${dir || city ? `<div class="flex items-start gap-2"><i class="fa-solid fa-location-dot text-gray-400 mt-0.5 w-4"></i><span class="text-gray-600 dark:text-text-dark-secondary">${[dir, city].filter(Boolean).join(', ')}</span></div>` : ''}
        ${tel ? `<div class="flex items-center gap-2"><i class="fa-solid fa-phone text-gray-400 w-4"></i><a href="tel:${tel}" class="text-indigo-600 hover:underline">${tel}</a></div>` : ''}
        ${mail ? `<div class="flex items-center gap-2"><i class="fa-solid fa-envelope text-gray-400 w-4"></i><a href="mailto:${mail}" class="text-indigo-600 hover:underline truncate">${mail}</a></div>` : ''}
      </div>
    `;
    centroCard.classList.remove('hidden');
  }
  ...
  function mostrarHistorialCuotasEnPantalla(cuotas, nombreCurso) {
    ...
    if (volverBtn) {
      volverBtn.onclick = () => {
        ...
        if (examFeeCard && !examFeeCard.classList.contains('hidden')) {
  ...
  if (curso.certificados && Array.isArray(curso.certificados)) {
    ...
    // Cursos autogestionables: mostrar mensaje en lugar de cards de certificado
    if (curso.autogestion == 1) {
      const mensajeCampus = `
        <div class="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl text-blue-800">
          <i class="fa-solid fa-graduation-cap text-2xl text-blue-500 flex-shrink-0"></i>
          <span class="text-sm font-medium">El certificado lo podés descargar desde el Campus Educativo</span>
        </div>`;

      const swiperWrapper = document.getElementById('swiper-certificates');
      if (swiperWrapper) {
        // Reemplazar el swiper por el mensaje, ocultando controles
        const swiperContainer = swiperWrapper.closest('.swiper');
        if (swiperContainer) {
          swiperContainer.innerHTML = mensajeCampus;
        } else {
          swiperWrapper.innerHTML = mensajeCampus;
        }
      }
      ...
      if (certificatesMobileList) certificatesMobileList.innerHTML = mensajeCampus;
      if (certificatesCount) certificatesCount.textContent = 'Ver en Campus';
      if (certificatesStatus) {
        certificatesStatus.textContent = 'Campus';
        certificatesStatus.className = 'px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800';
      }
    } else {
        ...
        if (examCuota.linkpago99 && examCuota.linkpago99.trim() !== '') {
          ...
          if (statuspagofacil) {
            ...
            payExamFeeButton.onclick = () => {
              window.open(examCuota.linkpago99, '_self');
              Swal.fire({
                title: 'Redirigiendo al pago',
                text: 'Serás redirigido al sistema de pagos.',
                icon: 'info',
                confirmButtonText: 'Entendido'
              });
            };
            ...
            payExamFeeButton.title = 'El sistema de pago no está disponible en este momento';
            payExamFeeButton.onclick = () => {
              Swal.fire({
                title: 'Pago no disponible',
                text: 'El sistema de pago no está disponible en este momento. Por favor, contacta con administración.',
                icon: 'warning',
                confirmButtonText: 'Entendido'
              });
            };
  ...
  function pagarCuota(cuota, curso) {
    console.log('[PAGO CUOTA] Iniciando pago de cuota regular:', cuota);
    console.log('[PAGO CUOTA] Tipo de pago:', curso.tipopago);
    
    const tipoPago = (curso.tipopago || '').toLowerCase();
    
    // Verificar si es SIRO o COIN
    if (tipoPago === 'siro' || tipoPago === 'coin') {
      // Verificar si existe linkpago en la cuota
      if (cuota.linkpago && typeof cuota.linkpago === 'object' && cuota.linkpago.data && cuota.linkpago.data.link) {
        console.log('[PAGO CUOTA] Usando link de pago:', cuota.linkpago.data.link);
        window.open(cuota.linkpago.data.link, '_blank');
        return;
      }
      
      // Si hay un link directo como string
      if (cuota.linkpago && typeof cuota.linkpago === 'string' && cuota.linkpago.trim() !== '') {
        console.log('[PAGO CUOTA] Usando link de pago directo:', cuota.linkpago);
        window.open(cuota.linkpago, '_blank');
        return;
      }
      
      // Si no hay link disponible
      console.error('[PAGO CUOTA] No se encontró link de pago para la cuota:', cuota);
      Swal.fire({
        title: 'Error',
      ...
      console.warn('[PAGO CUOTA] Tipo de pago no soportado:', tipoPago);
      Swal.fire({
        title: 'Método no disponible',
        text: 'El método de pago no está disponible. Por favor, contacta con administración.',
        icon: 'warning',
        confirmButtonText: 'Entendido'
      });
    }
  ...
  function pagarCuotaExamen(cuota) {
    ...
    if (cuota.linkpago99 && cuota.linkpago99.trim() !== '') {
      ...
      window.open(cuota.linkpago99, '_self');
      Swal.fire({
        title: 'Redirigiendo al pago',
        text: 'Serás redirigido al sistema de pagos.',
        icon: 'info',
    ...
    if (cuota.linkpago && cuota.linkpago.data && cuota.linkpago.data.link) {
      ...
      window.open(cuota.linkpago.data.link, '_self');
      showToast("Redirigiendo al sistema de pagos...", "info");
      return;
    }
