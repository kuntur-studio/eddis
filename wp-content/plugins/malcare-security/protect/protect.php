...
if (!class_exists('MCProtect_V636')) :
...
class MCProtect_V636 {
	...
	public static function init($mode) {
		...
		if ($mode == MCProtect_V636::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = MCProtectUtils_V636::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(MCProtect_V636::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new MCProtectRequest_V636($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			MCProtectFW_V636::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			MCProtectFW_V636::getInstance($mode, $request, $fw_config, $brand_name)->init();
			MCProtectLP_V636::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		MCProtectIpstore_V636::uninstall();
		MCProtectFW_V636::uninstall();
		MCProtectLP_V636::uninstall();

		MCProtect_V636::removeWPPrepend();
		MCProtect_V636::removePHPPrepend();
		MCProtect_V636::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		MCProtectUtils_V636::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		MCProtect_V636::removeHtaccessPrepend();
		MCProtect_V636::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		MCProtectUtils_V636::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		MCProtectUtils_V636::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		MCProtectUtils_V636::rrmdir($mc_data_dir);
	}
}
endif;