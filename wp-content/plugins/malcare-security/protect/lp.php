...
if (!class_exists('MCProtectLP_V636')) :
class MCProtectLP_V636 {
	private $ip;
	private $time;
	private $ipstore;
	private $logger;
	...
	private $mode              = MCProtectLP_V636::MODE_DISABLED;
	...
	private $category          = MCProtectLP_V636::CATEGORY_ALLOWED;
	private $username          = '';
	private $message           = '';
	...
	private function __construct($request, $config, $brand_name) {
		...
		$this->ipstore = new MCProtectIpstore_V636();
		$this->logger = new MCProtectLogger_V636(MCProtectLP_V636::TABLE_NAME);
	...
	public static function uninstall() {
		MCProtect_V636::$db->dropBVTable(MCProtectLP_V636::TABLE_NAME);
	}
	...
	private function getCaptchaLink() {
		$account = MCAccount::apiPublicAccount(MCProtect_V636::$settings);
	...
	private function getAllowLoginsTransient() {
		return MCProtect_V636::$settings->getTransient('bvlp_allow_logins');
	}

	private function getBlockLoginsTransient() {
		return MCProtect_V636::$settings->getTransient('bvlp_block_logins');
	}
	...
	private function isProtecting() {
		return $this->mode === MCProtectLP_V636::MODE_PROTECT;
	}

	private function isActive() {
		return $this->mode !== MCProtectLP_V636::MODE_DISABLED;
	}
	...
	private function isUnBlockedIP() {
		$transient_name = MCProtectLP_V636::UNBLOCK_IP_TRANSIENT_PREFIX . $this->ip;
		$attempts = MCProtect_V636::$settings->getTransient($transient_name);

		if ($attempts && $attempts > 0) {
			MCProtect_V636::$settings->setTransient($transient_name, $attempts - 1, 600 * $attempts);
			return true;
		}
	...
	private function isLoginBlocked() {
		if ($this->getAllowLoginsTransient() ||
				($this->getLoginCount(MCProtectLP_V636::LOGIN_STATUS_FAILURE, null, $this->all_blocked_gap) < $this->block_all_limit)) {
			return false;
		}
	...
	private function terminateLogin() {
		...
		$this->log(MCProtectLP_V636::LOGIN_STATUS_BLOCKED);
	...
	public function loginInit($user, $username = '', $password = '') {
		if ($this->isUnBlockedIP()) {
			$this->category = MCProtectLP_V636::CATEGORY_UNBLOCKED;
		} else {
			$failed_attempts = $this->getLoginCount(MCProtectLP_V636::LOGIN_STATUS_FAILURE,
																							$this->ip, $this->failed_login_gap);

			if ($this->isWhitelistedIP()) {
				$this->category = MCProtectLP_V636::CATEGORY_BYPASSED;
			} elseif (MCProtectUtils_V636::isPrivateIP($this->ip)) {
				$this->category = MCProtectLP_V636::CATEGORY_PRIVATEIP;
			} elseif ($this->isBlacklistedIP()) {
				$this->category = MCProtectLP_V636::CATEGORY_BLACKLISTED;
				$this->terminateLogin();
			} elseif ($this->isKnownLogin()) {
				$this->category = MCProtectLP_V636::CATEGORY_BYPASSED;
			} elseif ($this->isLoginBlocked()) {
				$this->category = MCProtectLP_V636::CATEGORY_ALL_BLOCKED;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->temp_block_limit) {
				$this->category = MCProtectLP_V636::CATEGORY_TEMP_BLOCK;
				$this->terminateLogin();
			} elseif ($failed_attempts >= $this->captcha_limit) {
				$this->category = MCProtectLP_V636::CATEGORY_CAPTCHA_BLOCK;
				$this->terminateLogin();
			}
		}
	...
	public function loginFailed($username) {
		...
		$this->log(MCProtectLP_V636::LOGIN_STATUS_FAILURE);
	}

	public function loginSuccess($username) {
		$this->username = $username;
		$this->message = 'Login Success';
		$this->log(MCProtectLP_V636::LOGIN_STATUS_SUCCESS);
	}

	private function isKnownLogin() {
		return $this->getLoginCount(MCProtectLP_V636::LOGIN_STATUS_SUCCESS,
																$this->ip, $this->success_login_gap) > 0;
	}

	private function getLoginCount($status, $ip, $gap) {
		$table = MCProtect_V636::$db->getBVTable(MCProtectLP_V636::TABLE_NAME);
		$query_str = "SELECT COUNT(*) as count from `$table` WHERE status=%d && time > %d";
		$query_args = array($status, ($this->time - $gap));

		$query = MCProtect_V636::$db->prepare($query_str, $query_args);
		if ($ip) {
			$query .= MCProtect_V636::$db->prepare(" && ip=%s", $ip);
		}

		$rows = MCProtect_V636::$db->getResult($query);
		if (!$rows) {
			return 0;
		}
