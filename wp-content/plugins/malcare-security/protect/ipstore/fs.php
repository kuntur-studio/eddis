...
if (!class_exists('MCProtectIpstoreFS_V636')) :
	class MCProtectIpstoreFS_V636 {
		...
		function __construct() {
			...
			$ips = MCProtectUtils_V636::parseFile($ip_store_file);
		...
		public function getTypeIfBlacklistedIP($ip) {
			return $this->getIPType($ip, MCProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPBlacklisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPWhitelisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V636::IP_TYPE_WHITELISTED);
		}
		...
		private function getIPType($ip, $type) {
			switch ($type) {
			case MCProtectIpstoreFS_V636::IP_TYPE_BLACKLISTED:
				return isset($this->blacklisted_ips[$ip]) ? $this->blacklisted_ips[$ip] : null;
			case MCProtectIpstoreFS_V636::IP_TYPE_WHITELISTED:
