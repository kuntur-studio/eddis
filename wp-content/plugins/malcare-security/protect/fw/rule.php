...
if (!class_exists('MCProtectFWRule_V636')) :
class MCProtectFWRule_V636 {
	public $id;
	public $logic;
	public $actions;
	public $execute_on;
	...
	public static function init($attributes) {
		...
		if (!array_key_exists('min_rule_engine_ver', $attributes) || !is_float($attributes['min_rule_engine_ver']) ||
				$attributes['min_rule_engine_ver'] > MCProtectFWRuleEngine_V636::VERSION) {

			return false;
		}

		if (array_key_exists('max_rule_engine_ver', $attributes) && (!is_float($attributes['max_rule_engine_ver']) ||
				$attributes['max_rule_engine_ver'] < MCProtectFWRuleEngine_V636::VERSION)) {
		...
		if (!array_key_exists('execute_on', $attributes)) {
			if (array_key_exists('level', $attributes) && $attributes['level'] === 1) {
				$attributes['execute_on'] = MCProtectFWRule_V636::EXE_ON_BOOT;
			} else {
				return false;
			}
		}

		return new MCProtectFWRule_V636($attributes);
	}

	public function isExeOnBoot() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_BOOT);
	}

	public function isExeOnPreUpdateOption() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_PRE_UPDATE_OPTION);
	}

	public function isExeOnPreDeletePost() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_PRE_DELETE_POST);
	}

	public function isExeOnWPInsertPostEmptyContent() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_WP_INSERT_POST_EMPTY_CONTENT);
	}

	public function isExeOnInsertUserMeta() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_INSERT_USER_META);
	}

	public function isExeOnDeleteOption() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_DELETE_OPTION);
	}

	public function isExeOnDeleteUser() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_DELETE_USER);
	}

	public function isExeOnPasswordReset() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_PASSWORD_RESET);
	}

	public function isExeOnSendAuthCookies() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_SEND_AUTH_COOKIES);
	}

	public function isExeOnSetAuthCookie() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_SET_AUTH_COOKIE);
	}

	public function isExeOnInit() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_INIT);
	}

	public function isExeOnUserRegister() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_USER_REGISTER);
	}

	public function isExeOnAddUserMeta() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_ADD_USER_META);
	}

	public function isExeOnUpdateUserMetadata() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_UPDATE_USER_METADATA);
	}

	public function isExeOnUpdateUserMeta() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_UPDATE_USER_META);
	}

	public function isExeOnAddOption() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_ADD_OPTION);
	}

	public function isExeOnWpPreInsertUserData() {
		return ($this->execute_on === MCProtectFWRule_V636::EXE_ON_WP_PRE_INSERT_USER_DATA);
	}
}
endif;