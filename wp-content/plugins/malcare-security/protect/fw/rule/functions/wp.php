...
if (!trait_exists('MCProtectFWRuleWPFunc_V636')) :
trait MCProtectFWRuleWPFunc_V636 {
	private function _rf_sanitizeUser() {
		...
		if (!function_exists('sanitize_user') || !MCProtectUtils_V636::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V636(
	...
	private function _rf_maybeSerialize() {
		...
		if (!function_exists('maybe_serialize') || !MCProtectUtils_V636::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V636(
	...
	private function _rf_isUserLoggedIn() {
		...
		if (!function_exists('is_user_logged_in') || !MCProtectUtils_V636::havePluginsLoaded()) {
			throw new MCProtectRuleError_V636(
	...
	private function _rf_getCurrentWPUser() {
		...
		if (!function_exists('wp_get_current_user') || !MCProtectUtils_V636::havePluginsLoaded()) {
			throw new MCProtectRuleError_V636(
				$this->addExState("getCurrentWPUser: Func wp_get_current_user doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V636::toAllowedType(wp_get_current_user());
	}
	...
	private function _rf_currentUserCan() {
		...
		if (!function_exists('current_user_can') || !MCProtectUtils_V636::havePluginsLoaded()) {
			throw new MCProtectRuleError_V636(
	...
	private function _rf_getUserBy() {
		...
		if (!function_exists('get_user_by') || !MCProtectUtils_V636::havePluginsLoaded()) {
			throw new MCProtectRuleError_V636(
		...
		return MCProtectFWRuleEngine_V636::toAllowedType($user);
	}
	...
	private function _rf_getCurrentWPUserCapabilities() {
		...
		return MCProtectFWRuleEngine_V636::toAllowedType($user["allcaps"]);
	}
	...
	private function _rf_getOption() {
		...
		if (!function_exists('get_option') || !MCProtectUtils_V636::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V636(
				$this->addExState("getOption: Func get_option doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V636::toAllowedType(get_option($option, $default_value));
	}
	...
	private function _rf_checkPasswordResetKey() {
		...
		if (!function_exists('check_password_reset_key') || !MCProtectUtils_V636::havePluginsLoaded()) {
			throw new MCProtectRuleError_V636(
	...
	private function _rf_wpUnslash() {
		...
		if (!function_exists('wp_unslash') || !MCProtectUtils_V636::haveMuPluginsLoaded()) {
			throw new MCProtectRuleError_V636(
